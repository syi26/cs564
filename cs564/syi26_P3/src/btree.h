...
class BTreeIndex {
 ...
 private:
  ...
  const void setEntry(Page *currPage, PageId pageNum,
                                  const RIDKeyPair<int> pairEnt, PageKeyPair<int> *&splitEnt, bool LorN);

  const void addToPage(NonLeafNodeInt *nonLeafNode, PageKeyPair<int> *currEntry);

  const void insertLeaf(LeafNodeInt *leafNode, RIDKeyPair<int> pairEnt);

  const void nonleaf(PageId &newNode, int key, NonLeafNodeInt *node);

  const void changeRoot(PageId oldPage, PageKeyPair<int> *splitEnt);

  const bool detectKey(int key, int lowVal, const Operator lowOp, int highVal, const Operator highOp);

  const int length_of_leaf(LeafNodeInt *node);
  const int length_of_nonleaf(NonLeafNodeInt *node);
  const int idx_in_array(const int *arr, int len, int key);
  const void insert_leaf(LeafNodeInt *node, int i, int key, RecordId rid);
  const void insert_nonleaf(NonLeafNodeInt *n, int i, int key, PageId pid);
